...
class NoiseInjection(object):
    ...
    def inject_noise_sample(self, data, noise_path, noise_level):
        noise_dst, _ = librosa.load(noise_path, self.sample_rate)
        noise_len = len(noise_dst)
        data_len = len(data)
        if noise_len > data_len:
            noise_start = np.random.randint(0, noise_len - data_len)
            noise_dst = noise_dst[noise_start: noise_start + data_len]
            noise_len = len(noise_dst)
        assert len(data) >= len(noise_dst), 'data length: {} vs. noise length: {}'.format(len(data), len(noise_dst))
        #noise_energy = np.sqrt(noise_dst.dot(noise_dst) / noise_dst.size)
        #data_energy = np.sqrt(data.dot(data) / data.size)
        if data_len > noise_len:
            start_point = np.random.randint(0, len(data) - len(noise_dst))
            data[start_point: start_point + noise_len] += noise_level * noise_dst #*  data_energy / noise_energy
        else:
    ...
    def __init__(self,
        ...
        if augmentation_conf and augmentation_conf.noise_dir:
            self.noise_injector = NoiseInjection(path=augmentation_conf.noise_dir,
                                                 ...
                                                 noise_levels=(augmentation_conf.noise_min, augmentation_conf.noise_max))
        else:
