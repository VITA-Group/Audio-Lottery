...
from deepspeech_pytorch.pruning_utils import pruning_model_random
...
class DeepSpeech(nn.Module):
    ...
    def load_model_package(cls, package):
        ...
        # temporary fix for the models that were saved before remove prune
        need_prune = False
        for k in package['state_dict']:
            if 'mask' in k:
                need_prune = True
                break
        # 0.8 is an arbitrary number for the model to generate tensors with mask and orig keys. The mask and the weight will be finally loaded from the checkpoint
        if need_prune:
            pruning_model_random(model, 0.8)
