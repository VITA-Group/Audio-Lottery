...
def evaluate(cfg: EvalConfig):
    ...
    prune_dirs = sorted(Path(cfg.model.model_path).glob('*'),
                        key=lambda x: int(x.name.split('_')[-1]))
    
    results = []
    for i, prune_dir in enumerate(prune_dirs):
        prune_index = int(prune_dir.name.split('_')[-1])
        model_path = prune_dir.joinpath('deepspeech_final.pth')
        ...
        wer, cer, output_data = run_evaluation(test_loader=test_loader,
                                               ...
                                               verbose=cfg.verbose,
                                               use_half=cfg.model.use_half)
        print('Pruning at {pt:.3f} \t'
              'Average WER {wer:.3f}\t'
              'Average CER {cer:.3f}\t'.format(pt=0.8**prune_index, wer=wer, cer=cer))
        results.append([0.8**prune_index, wer, cer])
        if cfg.save_output:
