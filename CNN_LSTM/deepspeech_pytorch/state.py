...
from deepspeech_pytorch.pruning_utils import remove_prune, prune_model_custom
...
class TrainingState:
    ...
    def serialize_state(self, epoch, iteration):
        ...
        if self.mask_dict is not None:
            remove_prune(model, model.rnn_type, model.bidirectional)
        model_dict = model.serialize_state()
        if self.mask_dict is not None:
            prune_model_custom(model, self.mask_dict, model.rnn_type, model.bidirectional)
