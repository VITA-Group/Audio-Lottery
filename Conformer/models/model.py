...
class Model(nn.Module):
    ...
    def load(self, path):
        ...
        # Mask dict
        if checkpoint["mask_dict"] is not None:
            self.mask_dict = checkpoint["mask_dict"]
            prune_model_custom(self, self.mask_dict)
    ...
    def lth_load(self, path, start_pt, initial_epoch=None):

        original_model_path = os.path.join(path, "prune_0", "checkpoints_0.ckpt")
        # Load Model Checkpoint
        if initial_epoch is not None:
            # if initial_epoch is not None, continue training of current sparsity
            prev_model_path = os.path.join(path, "prune_{}".format(start_pt), "checkpoints_{}.ckpt".format(initial_epoch))
        else:
            # if initial_epoch is None, start training of next sparsity
            prev_model_path = os.path.join(path, "prune_{}".format(start_pt - 1), "checkpoints_best.ckpt")
        self.load(prev_model_path)
        return original_model_path

    def prune_and_rewind(self, original_model_path, prune_percentage):
        checkpoint = torch.load(original_model_path, map_location=next(self.parameters()).device)
        self.mask_dict = prune_main(self, prune_percentage, checkpoint["model_state_dict"])
        self.optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
