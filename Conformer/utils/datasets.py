...
class LibriSpeechDataset(torch.utils.data.Dataset): 
    def __init__(self, dataset_path, training_params, tokenizer_params, split, args):
        ...
        self.names = glob.glob(dataset_path + split + "*/*/*/*.feat")
    ...
    def __getitem__(self, i):
        ...
        if self.lm_mode:
            return [torch.load(self.names[i].split(".feat")[0].split("_")[0] + "." + self.vocab_type + "_" + self.vocab_size)]
        else:
            return [torch.load(self.names[i])[0], torch.load(self.names[i].split(".feat")[0].split("_")[0] + "." + self.vocab_type + "_" + self.vocab_size)]
    ...
    def filter_lengths(self, audio_max_length, label_max_length, rank=0):
        ...
        return [name for name in self.names if torch.load(name + "_len") <= audio_max_length and torch.load(name.replace("feat", self.vocab_type + "_" + self.vocab_size + "_len")) <= label_max_length]
