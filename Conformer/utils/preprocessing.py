...
def prepare_dataset(training_params, tokenizer_params, tokenizer, encoder_params):
    ...
    if training_params["training_dataset"] == "LibriSpeech":
        ...
        for i, (sentence, label_path) in enumerate(zip(sentences, label_paths)):
            ...
            audio = torchaudio.load(label_path.split(".")[0] + ".flac")[0]
            audio_length = audio.size(1)
            feat, feat_len = audio_processor(audio, audio_length)

            # Save Feature and Feature length
            torch.save(torch.transpose(feat, 1, 2), label_path.split(".")[0] + ".feat")
            torch.save(feat_len.numpy().item(), label_path.split(".")[0] + ".feat_len")
